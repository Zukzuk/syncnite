...
async function findLibraryDir(root: string): Promise<string | null> {
...
// Find all directories under root that contain JSON files modified since sinceMs
 ...
 */
async function runImportCore(zipPath: string, password = "", isSync = false): Promise<void> {
...
export class BackupService {
    ...
    /**
     * Stores an uploaded file from tempPath to UPLOADS_DIR with a sanitized filename.
     * @param tempPath The temporary path of the uploaded file.
     * @param originalName The original filename of the uploaded file.
     * @returns The sanitized filename under which the file was stored.
     */
    ...
    /**
     * Processes a zip file from UPLOADS_DIR: extracts, dumps DB to JSON, merges into DATA_DIR.
     * @param params.filename The filename of the zip file in UPLOADS_DIR.
     * @param params.password Optional password for encrypted databases.
     */
